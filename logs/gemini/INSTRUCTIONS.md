...
Copy the **exact prompt** from `artifacts/prompt-template.md` (the text inside the ``` PROMPT ``` block) and paste it into Gemini.
...
Record the following in `logs/gemini/iteration-1.md`:
...
cp src/auth.js logs/gemini/auth.js
cp tests/auth.test.js logs/gemini/auth.test.js
```
...
2. **Do NOT** help Gemini beyond the initial prompt
...
1. Ensure all results are documented in `logs/gemini/iteration-1.md`
2. Ensure generated code is backed up to `logs/gemini/`
