# Claude Code Experiment Instructions

> **Note:** This document was created to ensure consistent experimental protocol across all three tools being evaluated.

**Purpose:** Run the Claude Code (Anthropic) portion of an AI coding assistant comparison study.
...
Copy the **exact prompt** from `artifacts/prompt-template.md` (the text inside the ``` PROMPT ``` block) and paste it into Claude Code.
...
Claude Code can write files directly to the filesystem when instructed.
...
Record the following in `logs/claude-code/iteration-1.md`:
...
cp src/auth.js logs/claude-code/auth.js
cp tests/auth.test.js logs/claude-code/auth.test.js
```
...
2. **Do NOT** help Claude Code beyond the initial prompt
...
1. Ensure all results are documented in `logs/claude-code/iteration-1.md`
2. Ensure generated code is backed up to `logs/claude-code/`
